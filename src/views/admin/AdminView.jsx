        ...
        <>
            ...
            <div className='w-full px-[5%] md:px-[10%] pt-[15vh] flex flex-col relative min-h-screen gap-16'>
                <div className="flex flex-col md:flex-row title2 gap-4">
                    ...
                    <Link className='underline text-primary'>{t("admin.createNewEvent")}</Link>
                </div>
                <div className="w-full flex flex-col md:flex-row gap-8 min-h-[60vh] ">
                    <div className="w-full  md:h-[65vh]  md:w-1/2 flex flex-col gap-4">
                        <span className='input flex rounded-full justify-between shadow-custom w-full '>
                            <input type="text" placeholder={t("admin.searchEvent")+"..."} className="w-full focus:no-underline focus:ring-0" />
                            ...
                            <div className="w-full  flex items-center font-roboto justify-between">
                                ...
                                <div className="w-full py-3 md:w-1/3 flex justify-center items-center rounded-full backdrop-filter relative backdrop-blur-md  ">
                                    ...
                                    <ul tabIndex={0} className={openFilters ? "shadow-custom rounded-2xl p-4 absolute w-full top-0 mt-16 bg-white transition-all" : "hidden"}>
                                        <li className="flex  hover:text-primary cursor-pointer" onClick={() => { setOpenFilters(false); setFilterSelected(t("admin.mostRecent")) }}>{t("admin.mostRecent")}</li>
                                        <li className="flex  hover:text-primary cursor-pointer" onClick={() => { setOpenFilters(false); setFilterSelected(t("admin.past")) }}>{t("admin.past")}</li>
                                        <li className="flex  hover:text-primary cursor-pointer" onClick={() => { setOpenFilters(false); setFilterSelected(t("admin.online")) }}>{t("admin.online")}</li>
                                        <li className="flex  hover:text-primary cursor-pointer" onClick={() => { setOpenFilters(false); setFilterSelected(t("admin.presential")) }}>{t("admin.presential")}</li>
                                    </ul>
                                </div>
                            </div>
                    ...
                    <div className="w-full  md:h-[65vh]  md:w-1/2 rounded-2xl shadow-custom p-4">
                        <span className='flex justify-between items-center'>
                            ...
                            <Link className='text-primary underline' to="/events/1" >{t("admin.viewEventsDetails")}</Link>
                        </span>
                        <span className='divider'></span>
